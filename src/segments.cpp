...
std::string Segments::timestamp()
{
	...
	std::string icon = getStr("segments.timestamp.icon", "");
	std::string iconStr = (icon.length() ? (" " + icon) : icon);
	...
	return this->bg(bg) + sep + this->fg(fg) + iconStr + std::string(buf) + " ";
}
...
std::string Segments::username()
{
	...
	std::string icon = getStr("segments.username.icon", "");
	std::string iconStr = (icon.length() ? (" " + icon) : icon);
	...
	return this->bg(bg) + sep + this->fg(fg) + iconStr + " \\u " + this->fg(bg);
}
...
std::string Segments::hostname()
{
	...
	std::string icon = getStr("segments.hostname.icon", "");
	std::string iconStr = (icon.length() ? (" " + icon) : icon);
	...
	return this->fg(fg) + sep + this->bg(bg) + " " + iconStr + " \\h " + this->fg(bg);
}
...
std::string Segments::cwd()
{
	...
	std::string icon = getStr("segments.cwd.icon", "");
	std::string iconStr = (icon.length() ? (" " + icon) : icon);
	...
	return this->bg(bg) + sep + this->fg(fg) + iconStr + " " + cwd + " " + this->fg(bg);
}
...
std::string Segments::venv()
{
	std::string sep = getStr("segments.venv.separator", this->angular);
	std::string fg = getStr("segments.venv.fg", "250");
	std::string bg = getStr("segments.venv.bg", "240");
	std::string icon = getStr("segments.venv.icon", "");
	std::string iconStr = (icon.length() ? (" " + icon + "  ") : icon);
	bool venv = false;

	// get cwd for comparing paths
	char *buffer;
	int bufSize = 1000;
	buffer = (char *)malloc(bufSize);
	getcwd(buffer, bufSize);
	...
	// get location of python binary
	std::string pythonCmd = "which python";
	std::string python;
	if (executeCmd(&python, pythonCmd, 100))
	{
		venv = (!python.compare(0, cwd.length(), cwd));
	}

	if (venv)
	{
		return this->bg(bg) + sep + this->fg(fg) + iconStr + this->fg(bg);
	}
...
std::string Segments::prompt()
{
	...
	std::string fg = getStr("segments.prompt.main.fg", "255");
	std::string bg = getStr("segments.prompt.main.bg", "240");
	std::string tbg = getStr("segments.prompt.trailing.bg", "240");
	...
	return this->bg(bg) + sep + this->fg(fg) + " " + prompt + " " + this->fg(bg) + this->bg(tbg) + sep + this->fg(tbg);
}
